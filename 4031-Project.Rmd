...
data = merge(data, School_Expenditure, by.x="School.Name", by.y="schoolname")
data = merge(data, Poverty.Percentage, by.x="School.Name", by.y="SCHOOL_NAME")
data = merge(data, Mobility, by.x="School.Name", by.y="school_name")
data = merge(data, Enrollment, by.x="School.Name", by.y="INSTN_NAME")
attach(data)
...
data$growth.rate.math <-data$X19.21.Difference.in.Math / data$All.Students.Math.Achievement
```
...
```{r, echo=FALSE}
library(huxtable)
ht <- hux(firstcol = c('', 'Avg. Math achievement', 'Median Math achievement', 
                       'Lower Bound of Math achievement',
                       'Upper Bound of Math achievement'),
          a2019 = c('2019', mean_math_2019, median_math_2019,
                    range_math_2019_lower, range_math_2019_upper),
          a2021 = c('2021', mean_math_2021, median_math_2021,
                    range_math_2021_lower, range_math_2021_upper),
          add_colnames = FALSE)
bold(ht)[1,]           <- TRUE
align(ht)[,2]          <- "left"
font_size(ht)          <- 9
...
head(theme_plain(ht), 5L)
```

Avg. Math achievement across all students in 2019: $\fbox{67.99686463672}$.\
Median Math achievement across all students in 2019: $\fbox{69.34}$.\
Range of Math achievement across all students in 2019: $\fbox{[2.88, 100]}$.\

Avg. Math achievement across all students in 2021: $\fbox{52.2312059089539}$.\
Median Math achievement across all students in 2021: $\fbox{52.24}$.\
Range of Math achievement across all students in 2021: $\fbox{[0, 100]}$.
...
h1 <-hist(data$All.Students.Math.Achievement,main = "2019 Math Achievement Rate",xlab="2019 All Students Math Achievement Rate", 
     ylim = c(0,900), breaks=10,xlim = c(0,100))
...
#2021 normal curve
h2 <- hist(data$X2021.All.Students.Math.Achievement,main = "2021 Math Achievement Rate", xlab = "2021 All Students Math Achievement Rate",
     ylim = c(0,900), breaks=10,xlim = c(0,100))
...
for (i in 1:(ncol(data_numeric)-2)) {
  quartiles <- quantile(data_numeric[ ,i], probs=c(.25, .75), na.rm=FALSE)
  IQR <- IQR(data_numeric[ ,i])
...
corrplot(cor(data_numeric[,!names(data_numeric)
                          %in% c("x2", "x3","x4","x6","x8","x9","x10","x11")],
...
```{r, echo=FALSE}
# Multicollinearity Renewed
library(dplyr)
library(corrplot)
# par(mfrow=c(1,2))
# corrplot(cor(data_numeric[,!names(data_numeric) %in% c("x3","x4","x6","x8","x9","x10","x11")],
...
full_model <- regsubsets(y1~x1+x5+x7+x12+x13+x14+x15+x16+x17+x18+x19+x20,
...
library(leaps)
par(mfrow=c(1, 3))
all_model_2021 <- regsubsets(y2~x1+x5+x7+x12+x13+x14+x15+x16+x17+x18+x19+x20,
                             data=data_numeric, nbest=1, method = "exhaustive")
